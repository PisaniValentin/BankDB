...
public class DAOClienteMorosoImpl implements DAOClienteMoroso {
	...
	public ArrayList<ClienteMorosoBean> recuperarClientesMorosos() throws Exception {
		...
		*/
		...
		try {
			String sql = "SELECT nro_prestamo,count(nro_prestamo) AS cuotas_vencidas FROM pago NATURAL JOIN prestamo WHERE fecha_pago is null and fecha_venc < curdate() GROUP BY nro_prestamo HAVING cuotas_vencidas>=2";
			...
			while (rs.next()) {
				...
				moroso.setCantidadCuotasAtrasadas(rs.getInt("cuotas_vencidas"));
				morosos.add(moroso);
			}
			return morosos;
